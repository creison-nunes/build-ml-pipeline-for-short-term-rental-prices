...
def go(args):
    ...
    ######################################
    # Use download() instead of .file() to avoid Windows path issues with colons
    artifact_dir = run.use_artifact(args.trainval_artifact).download(
        root=os.path.join(".", "artifacts", "trainval")
    )
    ...
    if os.path.exists("random_forest_dir"):
        # On Windows, some files may be read-only; force removal by resetting permissions first.
        def _force_remove(func, path, _):
            os.chmod(path, stat.S_IWRITE)
            func(path)
        shutil.rmtree("random_forest_dir", onerror=_force_remove)
